...
class Goods extends BaseModel {
     ...
     * @return type
     */
    public function addGoodsPifa($data) {
        return Db::name('goods_pifa')->insertGetId($data);
    }
...
//        $condition = $this->_getRecursiveClass($condition);
     ...
     * @return array
     */
    public function getMobileGoodsList($condition, $field = '*', $group = '', $order = '', $limit = 0, $pagesize = 0, $lock = false) {
        $condition[]=array('goods_state','=',self::STATE1);
        $condition[]=array('goods_verify','=',self::VERIFY1);
        
        $result = Db::name('goods')->field($field)->where($condition)->page($pagesize,$limit)->group($group)->order($order)->select()->toArray();
        return $result;
    }
...
//        $condition = $this->_getRecursiveClass($condition);
    ...
    public function getGoodsCommendList($store_id, $limit = 5) {
        $goods_commend_list = $this->getGoodsOnlineList(array(array('store_id' ,'=', $store_id), array('goods_commend' ,'=', 1)), 'goods_id,goods_commonid,goods_name,goods_advword,goods_image,store_id,goods_promotion_price,goods_price', 0, '', $limit, 'goods_commonid');
        if (!empty($goods_id_list)) {
            $tmp = array();
            foreach ($goods_id_list as $v) {
                $tmp[] = $v['goods_id'];
            }
            $goods_commend_list = $this->getGoodsOnlineList(array(array('goods_id','in', $tmp)), 'goods_id,goods_commonid,goods_name,goods_advword,goods_image,store_id,goods_promotion_price', 0, '', $limit);
        }
     ...
     * @return boolean|unknown
     */
    public function editGoodsPifa($update, $goods_id) {
        if (empty($goods_id)) {
            return true;
        }
        $result = Db::name('goods_pifa')->where('goods_id','=',$goods_id)->update($update);
        return $result;
    }
    ...
    public function getGoodsInfoAndPromotionById($goods_id) {
        ...
        $goods_commoninfo = $this->getGoodsCommonInfo(array(array('goods_commonid', '=', $goods_info['goods_commonid'])));
        if($goods_commoninfo['goods_ispifa'] > 0){
        	$goods_info['goods_ispifa'] = $goods_commoninfo['goods_ispifa'];
        	$goods_pifainfo = $this->getGoodsPifaInfo(array(array('goods_id', '=', $goods_info['goods_commonid'])));
	        if(!empty($goods_pifainfo)){
	        	unset($goods_pifainfo['id']);
	        	unset($goods_pifainfo['goods_id']);
	        	$goods_info = array_merge($goods_info, $goods_pifainfo);
	        }else{
	        	$goods_info['startnum1'] = 0;
	        	$goods_info['endnum1'] = 0;
	        	$goods_info['pifaprice1'] = 0;
	        	$goods_info['startnum2'] = 0;
	        	$goods_info['endnum2'] = 0;
	        	$goods_info['pifaprice2'] = 0;
	        	$goods_info['startnum3'] = 0;
	        	$goods_info['endnum3'] = 0;
	        	$goods_info['pifaprice3'] = 0;
	        }
        }else{
        	$goods_info['startnum1'] = 0;
        	$goods_info['endnum1'] = 0;
     ...
     * @return array
     */
    public function getGoodsPifaInfo($condition, $field = '*') {
        return Db::name('goods_pifa')->field($field)->where($condition)->find();
    }
    ...
    public function getGoodsDetail($goods_id) {
        ...
        $result3 = $this->getGoodsPifaInfo(array(array('goods_id', '=', $result1['goods_commonid'])));
        $goods_info = array_merge($result2, $result1);
        if(!empty($result3)){
        	unset($result3['id']);
        	unset($result3['goods_id']);
        	$goods_info = array_merge($goods_info, $result3);
        }else{
        	$goods_info['startnum1'] = 0;
        	$goods_info['endnum1'] = 0;
        	$goods_info['pifaprice1'] = 0;
